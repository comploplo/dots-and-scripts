...
local write_count = 0
-- local git_branch = ""
-- local status_line = [[%f %y %m %= %p%% %l:%c %{get(b:,'gitsigns_status','')}]]
local status_line = [[%y %m %= %l:%c %{get(b:,'gitsigns_status','')}]]
...
local statusline = "%%-4.4(%d%%)%%50.23(%s%%)|%%5.14(%s%%)%%5.20(%s%%)"
-- local statusline = "%d%%  | %%8.0(%s%%) | %s%% | %s%%"
function StatusLine()
